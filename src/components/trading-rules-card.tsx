...
// Tab value type
type TabValue = "selfcheck" | "position" | "strategy" | "timewindow" | "stoploss" | "volume" | "voldown";

// Which RuleKeys are relevant for each tab (for indicator badges)
const tabRuleMap: Record<TabValue, RuleKey[]> = {
  selfcheck: [],
  position: ["vol_decline_position"],
  strategy: [],
  timewindow: [],
  stoploss: [],
  volume: ["vol_decline_danger", "vol_surge_strong", "pulse_caution", "early_vol_drop", "wash_trade", "shrink_rise", "shrink_rise_warn", "vol_rise", "slow_decline"],
  voldown: ["vol_decline_danger", "pulse_decline_danger"],
};
...
export const TradingRulesCard = React.memo(function TradingRulesCard({ autoExpanded, pvMarkers = [] }: TradingRulesCardProps) {
  ...
  // Determine default tab based on danger signals
  const getDefaultTab = (): TabValue => {
    if (hasDangerSignal) return "voldown";
    // Volume-related triggers
    const volumeKeys = tabRuleMap.volume;
    if (volumeKeys.some(k => activeRules.has(k))) return "volume";
    return "selfcheck";
  };

  const [activeTab, setActiveTab] = useState<TabValue>("selfcheck");

  // Compute preferred tab based on danger signals
  const preferredTab = useMemo<TabValue>(() => {
    if (hasDangerSignal) return "voldown";
    if (tabRuleMap.volume.some(k => activeRules.has(k))) return "volume";
    return "selfcheck";
  }, [hasDangerSignal, activeRules]);

  // When toggling expand, auto-select preferred tab on open
  ...
  // Helper: check if a tab has any active rule triggers
  const tabHasTrigger = (tab: TabValue): boolean => {
    return tabRuleMap[tab].some(k => activeRules.has(k));
  };
    ...
    <Card className={`border-2 shadow-lg mb-4 ${hasDangerSignal ? "border-red-500/60 border-l-4 border-l-red-500 shadow-red-500/15 bg-gradient-to-br from-red-500/5 via-transparent to-transparent" : "border-amber-500/40 border-l-4 border-l-amber-500 shadow-amber-500/10 bg-gradient-to-br from-amber-500/5 via-transparent to-transparent"}`}>
      ...
      {expanded && <CardContent className="pt-0">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as TabValue)}>
          <TabsList className="h-7 w-full mb-2 flex-wrap gap-0.5 overflow-x-auto p-0.5">
            <TabsTrigger value="selfcheck" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-amber-500/15 data-[state=active]:text-amber-700 dark:data-[state=active]:text-amber-300">
              <span className="text-xs">🎯</span> 自检
            </TabsTrigger>
            <TabsTrigger value="position" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-red-500/15 data-[state=active]:text-red-700 dark:data-[state=active]:text-red-300">
              <AlertTriangle className="w-3 h-3" /> 仓位
              {tabHasTrigger("position") && <span className="text-red-500 text-[9px] leading-none">⚠</span>}
            </TabsTrigger>
            <TabsTrigger value="strategy" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-violet-500/15 data-[state=active]:text-violet-700 dark:data-[state=active]:text-violet-300">
              <Zap className="w-3 h-3" /> 策略
            </TabsTrigger>
            <TabsTrigger value="timewindow" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-cyan-500/15 data-[state=active]:text-cyan-700 dark:data-[state=active]:text-cyan-300">
              <Clock className="w-3 h-3" /> 时间
            </TabsTrigger>
            <TabsTrigger value="stoploss" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-rose-500/15 data-[state=active]:text-rose-700 dark:data-[state=active]:text-rose-300">
              <AlertTriangle className="w-3 h-3 text-rose-500" /> 止损
            </TabsTrigger>
            <TabsTrigger value="volume" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-amber-500/15 data-[state=active]:text-amber-700 dark:data-[state=active]:text-amber-300">
              <Volume2 className="w-3 h-3" /> 量能
              {tabHasTrigger("volume") && <span className="text-red-500 text-[9px] leading-none">⚠</span>}
            </TabsTrigger>
            <TabsTrigger value="voldown" className="text-[10px] px-2 h-5 rounded-full gap-0.5 data-[state=active]:bg-red-500/15 data-[state=active]:text-red-700 dark:data-[state=active]:text-red-300">
              <AlertTriangle className="w-3 h-3 text-red-500" /> 放量下跌
              {tabHasTrigger("voldown") && <span className="text-red-500 text-[9px] leading-none animate-pulse">⚠</span>}
            </TabsTrigger>
          </TabsList>
        </Tabs>
