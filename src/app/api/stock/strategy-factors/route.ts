...
const DEFAULT_FACTOR_SEEDS = [
  ...
  // v4.5 new factor
  { name: "放量下跌买点", category: "VOLUME_DECLINE_BUY", signalType: "buy", description: "放量下跌后的买点：①MACD绿柱近期达峰值(>=60%)②成交量缩量到地量(<50%均量)③当前价格挨着最低价(0.5%内)（v4.6）", params: JSON.stringify({ macdPeakRatio: 0.6, volDryUpRatio: 0.5, priceNearLowPct: 0.5 }), enabled: true, priority: 24, strength: "strong", tMode: "正T", timeWindow: "buy_window" },
];
...
async function seedDefaultData() {
  const defaultFactors = [
    ...
    { name: "放量下跌买点", category: "VOLUME_DECLINE_BUY", signalType: "buy", description: "放量下跌后的买点：①MACD绿柱近期达峰值(>=60%)②成交量缩量到地量(<50%均量)③当前价格挨着最低价(0.5%内)（v4.6）", params: JSON.stringify({ macdPeakRatio: 0.6, volDryUpRatio: 0.5, priceNearLowPct: 0.5 }), enabled: true, priority: 24, strength: "strong" },
  ];
