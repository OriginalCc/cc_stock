 ...
 * POST /api/stock/prediction-history          — 保存今日预测
...
async function verifyPredictions() {
  ...
  // Get unique sector codes to fetch their data
  const sectorCodes = [...new Set(unverified.map(r => r.sectorCode))];

  // Fetch current sector data from eastmoney to get actual change%
  const actualData = new Map<string, { changePercent: number; mainNetInflow: number }>();

  // Fetch industry sectors
  try {
    const url = `http://push2delay.eastmoney.com/api/qt/clist/get?pn=1&pz=100&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:90+t:2&fields=f3,f12,f62`;
  ...
  try {
    const url = `http://push2delay.eastmoney.com/api/qt/clist/get?pn=1&pz=100&po=1&np=1&fltt=2&invt=2&fid=f3&fs=m:90+t:3&fields=f3,f12,f62`;
    const resp = await fetch(url, { next: { revalidate: 0 }, signal: AbortSignal.timeout(10000) });
    if (resp.ok) {
      const data = await resp.json();
      const diff = data?.data?.diff;
  ...
  return NextResponse.json({
    ...
    message: `已验证 ${verified}/${unverified.length} 条预测`,
  });
}
